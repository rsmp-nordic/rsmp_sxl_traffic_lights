...
uses a selected signal program during normal control to operate signal groups. Each
...
request a functional position, select a signal program, adjust exposed timing
...
* Configuration and live operation are different. Signal-group mappings,
  detector logic, signal programs, schedules, identifiers, and safety rules are
...
* A selected or requested value is not necessarily the currently observed
  state. A selected signal program, requested functional position, observed control
  mode, and current signal-group status describe different aspects of
  operation.
* Safety functions constrain all control decisions. signal programs, local demand,
...
``Detector logic``
   A logical function that interprets one or more physical detector inputs and
   produces logical output, eg demand. Detector logics are used to
   affect signal timing or change or signal programs.
...
``Controller programming``
   ...
   demand, apply signal programs, and determine signal group changes. SXL exposes
...
``Intergreen``
   ...
   group and the start of permission for a conflicting signal group.
   Typically defined by national or local regulations.
...
``signal program``
   ...
   timings. It can define sequence, cycle length, coordination behavior, and
...
``Day table``
   A local schedule that selects signal programs at configured times of day. A time
   table is not a signal program: the table says *when to select*, while the plan
...
``Traffic situation``
   A defined set of traffic circumstances, often traffic volumme, events or emergency situations.
   Some controllers can be configured to select a signal program based on the current
   traffic situation.

An external system can request a particular signal program or return selection to
...
``Offset``
   A time displacement between the base cycle counter and coordinated cycle counter.
   When several controlers use the same cycle length, the offset can be used to shift
   the start of the cycle relative to other controllers, for example to create a
   green wave along a corridor.

``Split``
   The allocation of green time within a cycle to different signal groups.
...
``Traffic-actuated control``
   ...
   extend green within the configured limits of the selected signal program.
...
``Adaptive control``
   ...
   may select or adjust fixed signal programs, but the SXL defines only the
...
``Control mode``
   The overall operational condition of an intersection, such as startup,
   normal control, or failure.
